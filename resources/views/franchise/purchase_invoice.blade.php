                                      ...
                                      <tbody>
                                        ...
                                        <tr>
                                            <td style="font-size: 12px; color: #5b5b5b; font-family: 'Open Sans', sans-serif; line-height: 18px; vertical-align: top;;">
                                                    ...
                                                    <small>Invoice No. </small> #&nbsp;<b><?php echo $r_orders['0']->order_id; ?></b><br />
                    ...
                    <table width="1000" border="0" cellpadding="0" cellspacing="0" align="center" class="fullTable" bgcolor="#ffffff">
                      <tbody>
                        ...
                        <?php
                            ...
                            $product_details = json_decode($r_orders['0']->products,1);
                            
                            foreach($product_details as $rows){
                                
                                $productss=DB::table('products')->where('id' , $rows['id'])->get();
                                $usrss=DB::table('user')->where('id' , $r_orders['0']->user_id)->get();
                                
                                $product_name=$rows['name'];
                                $sku_code=$productss['0']->sku_code;
                                $price=$rows['mrp'];
                                $business_value=$rows['business_value'];
                                $total_business_value += $business_value;
                                $quantity=$rows['quantity'];
                                $total_quantity +=$quantity;
                                $total_price= $quantity*$price;
                                
                                if(!empty($rows['percent_discount'])){
                                    $DP=(($price*$rows['percent_discount'])/100);
                                    $total=($price-($price*$rows['percent_discount'])/100)*$quantity;
                                    $perquantityDiscount=$price-($price*$rows['percent_discount'])/100;
                                    $discount=$rows['percent_discount'].'%';
                                }
                                
                                if(!empty($rows['flat_discount'])){
                                    $DP=$rows['flat_discount'];
                                    $total=($price-$rows['flat_discount'])*$quantity;
                                    $perquantityDiscount=$price-$rows['flat_discount'];
                                    $discount=$rows['flat_discount'];
                                }
                                ...
                                $subtotal+=$total;
                                $totalshipping=$r_orders['0']->Totalshipping_charge;
