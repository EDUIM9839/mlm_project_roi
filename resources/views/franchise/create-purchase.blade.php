                ...
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><?php echo 'New Purchase'; ?></li>
                        </ol>
                    </nav>
                </div>
         ...
         <div class="col-md-6" style="background-color:rgb(243 243 243);">
               ...
               <div class="row hidden-md-up">
                  <?php 
                      foreach( $data as $row){
                         ...
                         $product_stocks=DB::table('product_stocks')->where('pid', $row->product_id)->get();
                  ...
                  ?> 
                  <div class="col-md-4"  onclick="fbilling_add_to_cart('<?php echo $row->product_id;?>');">
                     ...
                     <div class="col">
						<div class="card">
						    <div class='row'>
						        <div class='col-md-6'>
        						     <?php if($row->quantity<3){?>
                                       <span class="badge bg-danger hidestockspan<?php echo $row->product_id;?>" style="font-size:8px">QTY : {{$row->quantity}}</span>
                                     <?php }else{ ?>
                                       <span class="badge bg-success" style="font-size:8px">QTY : {{$row->quantity}}</span>
                                      <?php } ?>
                                      <span class="badge bg-danger" style="font-size:8px" id='outofStock<?php echo $row->product_id;?>'></span>
                                </div>
                                
                                <div class='col-md-6'>
                                    
                                        @if(!empty($products['0']->percent_discount))
                                            <div><span style='float:right;margin-right:2px;font-size:8px;margin-top:5px' class="badge bg-info">-{{$products['0']->percent_discount}}&nbsp;% &nbsp;OFF</span></div>
                                         @else
                                            <div><span style='float:right;margin-right:2px;font-size:8px;margin-top:5px' class="badge bg-info">- {{Helper::get_currency()}}&nbsp;{{$products['0']->flat_discount}}&nbsp;OFF</span></div>
                                         @endif
							...
							<div class="card-body">
								<h6 class="card-title cursor-pointer" style='font-size:12px'>{{$products['0']->product_name}}</h6>
								<div class="clearfix">
									<p class="mb-0 float-start" style='font-size:10px'><strong style='font-size:10px'>{{$products['0']->business_value}}</strong> BV</p>
									<p class="mb-0 float-end fw-bold"><span class="me-2 text-decoration-line-through text-secondary">{{Helper::get_currency()}}&nbsp;{{$products['0']->mrp}}</span><span>{{Helper::get_currency()}}&nbsp;{{$products['0']->dp}}</span></p>
								</div>
								<div class='row'>
    								<div class="d-flex align-items-center mt-3 fs-6">
    								  <!--<div class='col-md-12'><button class='btn btn-warning' onclick="add_to_cart('<?php //echo $row->id;?>');" style='float:right;background: #fb641b;box-shadow: 0 1px 2px 0 rgba(0,0,0,.2);border: none;color: #fff;width:100%;font-size:12px'>Add To Cart</button></div>-->
    								</div>
								</div>
							</div>
            ...
            <form id='myform' enctype='multipart/form-data'>
                ...
                <div>
                      ...
                      <div class="position-relative input-icon mt-2">
							<input type="text" class="form-control" onkeyup="search_franchises();" onchange="search_franchises();"   <?php if(!empty($user['0']->id)){?> value='{{$user["0"]->first_name}}' <?php } ?> onmousedown="search_franchises();"  autocomplete='off'   name="customer" id="customer">
						    ...
						    <?php if(!empty($user['0']->id)){?>
						     ...
						     <?php } else{ ?>
						        ...
						        <span class='badge bg-success' id='hiddenwallet'  style='text-align: left;font-size: 12px;width: 170px;color: black;display: none;margin-top: 2px '  >Wallet Amount :{{Helper::get_currency()}}&nbsp;<small id='savingwallet'></small></span>
						     
						     <?php } ?>
					  </div>
					  <label id='customer_error' style="color:red"></label>
                      <ul  style="display:none1;margin-top: -20px " class="list-group" id="lidata"></ul>
                </div>
                  ...
                  <table class="table">
                     ...
                     <tbody id="tablerow">
                        ...
                        <tr>
                            ...
                            <td style="text-align:center"><a onclick="fbilling_decreaseqty('{{$rows['id']}}')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px" type="text" min="1" value="{{$rows['quantity']}}" id="" readonly ><a onclick="fbilling_increaseqty('{{$rows['id']}}')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a></td>
                            ...
                            <td style="text-align:center"><a onclick="fbilling_delete_Cart_Item({{$rows['id']}})"><i class="fa fa-trash"></i></a></td>
                     ...
                     <tfoot id='tfooter'>
                        <tr>
                            ...
                            <?php  if(!empty($user['0']->id)){?><td style="text-align:center"><input type="hidden" value='{{$subtotal}}'  id="total" name="total"></td><?php } ?>
                        </tr>
                     </tfoot>
                  </table>
               </div>
               ...
               <div style="margin-top:10px;" id='countinue_sale'>
                  ...
                  <input type="button"  onclick="fbilling_uploadData();" style="float:right;" class="btn btn-primary"  class="fixed-bottom" value="Continue Sale">
               </div>
               <?php } else { ?>
...
function search_franchises(){
            ...
            $.ajax({
            type: 'post',
            url: '{{ route("fbilling_searchFranchises") }}',
            data: {"sdata":searchdata,"from_id":from_id,"_token":"{{csrf_token()}}"},
            success: function (data) {
                $("#lidata").html(data);
                $("#lidata").css('display', 'block');
                $(".bgcolor").css('background-color', 'rgb(203 200 200)');
                $(".bgcolor").css('margin-top', '0px');
                $(".bgcolor").css('color', 'black');
            }
        }); 
       }
function fbilling_add_to_cart(id){
            var cid=$("#customer_id").val();
            var from_id=$("#from_id").val();
            $.ajax({
            type: 'post',
            url: '{{ route("fbilling_add_to_cart") }}',
                ...
                if(data==0){
                 ...
                 $('.hidestockspan'+id).css('display', 'none');
                    ...
                    '<td style="text-align:center">'+value.name+'</td>'+
                     '<td style="text-align:center">'+'<a onclick="fbilling_decreaseqty('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px" type="text" min="1" value="'+value.quantity+'" id="quantity'+value.id+'" readonly ><a onclick="fbilling_increaseqty('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a>'+'</td>'+
                    '<td style="text-align:center">'+value.mrp+'</td>'+
                    '<td style="text-align:center">'+dp+'</td>'+
                    '<td style="text-align:center">'+total+'</td>'+
                    '<td style="text-align:center">'+total_bv+'</td>'+
                     '<td style="text-align:center">'+'<a onclick="fbilling_delete_Cart_Item('+value.id+')"><i class="fa fa-trash"></i></a>'+'</td>'+
...
function fbilling_increaseqty(id){
            ...
            $.ajax({
            type: 'post',
            url: '{{ route("fbilling_increaseqty") }}',
            data: {'id':id, 'qty':qty,'cid':cid,'from_id':from_id, "_token":"{{csrf_token()}}"},
            success: function (data) {
                     cdata = JSON.stringify(data['cartdata']);
                     $("#allproducts").val(cdata);
                     var result  ='';
                     var res='';
                    var i=0;
                    ...
                    '<td style="text-align:center">'+i+'</td>'+
                    '<td style="text-align:center">'+value.name+'</td>'+
                    '<td>'+'<a onclick="fbilling_decreaseqty('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px;background-color:#e7e7e7" type="text" min="1" value="'+value.quantity+'" id="quantity'+value.id+'" readonly ><a onclick="fbilling_increaseqty('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a>'+'</td>'+
                    '<td style="text-align:center">'+value.mrp+'</td>'+
                    '<td style="text-align:center">'+dp+'</td>'+
                    '<td style="text-align:center">'+total+'</td>'+
                    '<td style="text-align:center">'+total_bv+'</td>'+
...
function fbilling_decreaseqty(id){
           ...
           var from_id=$("#from_id").val();
            $.ajax({
            type: 'post',
            url: '{{ route("fbilling_decreaseqty") }}',
            data: {'id':id, 'qty':qty,'cid':cid,'from_id':from_id, "_token":"{{csrf_token()}}"},
            success: function (data) {
                   var cdata = JSON.stringify(data['cartdata']);
                   ...
                   '<td style="text-align:center">'+i+'</td>'+
                    '<td style="text-align:center">'+value.name+'</td>'+
                    '<td style="text-align:center">'+'<a onclick="fbilling_decreaseqty('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px;background-color:#e7e7e7" type="text" min="1" value="'+value.quantity+'" id="quantity'+value.id+'" readonly ><a onclick="fbilling_increaseqty('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a>'+'</td>'+
                    '<td style="text-align:center">'+value.mrp+'</td>'+
                    '<td style="text-align:center">'+dp+'</td>'+
                    '<td style="text-align:center">'+total+'</td>'+
                    '<td style="text-align:center">'+total_bv+'</td>'+
                    '<td style="text-align:center">'+'<a onclick="fbilling__delete_Cart_Item('+value.id+')"><i class="fa fa-trash"></i></a>'+'</td>'+
...
function fbilling_delete_Cart_Item(id){
            var cid=$("#customer_id").val();
            var from_id=$("#from_id").val();
            var result  ='';
            $.ajax({
            type: 'post',
            url: '{{ route("fbilling_delete_Cart_Item") }}',
             data: {'id':id,'cid':cid,'from_id':from_id, "_token":"{{csrf_token()}}"},
            success: function (data) {
                 var cdata = JSON.stringify(data['cartdata']);
           ...
           $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data=='1'){
                                ...
                                var url = "{{ route('fbilling_forgetcartItems') }}";
                                setTimeout(function () {
                                ...
                                var url = "{{ route('cashpayment') }}";
                                window.location.href = url;
                         }else if(data=='19'){
                            
                                var url = "{{ route('onlinepayment') }}";
                                window.location.href = url;
...
function selectValsss(id){
                ...
                $("#wallet").text($("#saveWallet"+id).val());
                 $("#savingwallet").text($('#saveWallet'+id).val());
               $("#hiddenwallet").css('display', 'block');
