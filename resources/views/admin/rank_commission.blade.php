...
@section('pageTitle', 'Rank Commission')
@section('mains')
...
<!-- Gif end -->
 ...
 <div class="card-body p-4">
      <div align="right" class="btn-container justify-content-end mt-2">
      <a  data-id="#add_rank" data-target="#add_rank" data-toggle="modal"  class="btn btn-outline-info" style="margin: 10px;">Add Rank &nbsp;+</a>
      </div>
    <table id="customers">
        <tbody>
            <tr>
            ...
            <tr>
                ...
                <td>{{$item->target}}</td>
                <td>{{$item->percent}}</td>
                <td><img src="{{ Storage::url('app/rankImages/').$item->medals}}" style="height: 50px;width:70px;"></td>
                <td id="edit"><a onclick="geteditid({{$item->id}});" data-target="#sidebar-right" data-toggle="modal"  class="btn btn-primary navbar-btn pull-left">Edit<i class="bx bx-edit"></i></a>&nbsp;&nbsp;
                <a type="button" class="btn btn-outline-danger btn-sm" onclick="return confirm('Delete this record?')" href="{{route('delete',['id'=>$item->id])}}">
...
<form action="{{route('update_rank')}}" method="POST" enctype="multipart/form-data">
 ...
 <input type="hidden" class="form-control" style="border-color:#afa7a7;" 
 value="<?php if(!empty($data['0']->id)){?>{{ $data['0']->medals }}<?php } ?>" name="rank_oldimage"  id="rank_oldimage">
 <div class="border p-4 rounded">
    ...
    $("#editid").val(id);
                ...
                $.ajax({
                    ...
                    success : function(response) {
                       ...
                       $("#rank_oldimage").val(obj.medals);
                       var imgPath='{{Storage::url('app/rankImages/')}}'+"/"+obj.medals; 
