         ...
         <div class="col-md-6" style="background-color:rgb(243 243 243);">
            <div class="container" style="margin-top: 15px">
               <div class="row hidden-md-up">
                  <?php 
                      foreach( $data as $row){
                         $product_stocks=DB::table('product_stocks')->where('pid', $row->id)->get();
                         $product_images=DB::table('product_images')->where('pid', $row->id)->get();
                        
                  ?> 
                  <div class="col-md-4"  onclick="add_to_cart('<?php echo $row->id;?>');">
                    
                     <div class="col">
						<div class="card">
						    <div class='row'>
						        <div class='col-md-6'>
        						     <?php if($product_stocks['0']->stock<3){?>
                                       <span class="badge bg-danger hidestockspan<?php echo $row->id;?>" style="font-size:8px">QTY : {{$product_stocks['0']->stock}}</span>
                                     <?php }else{ ?>
                                       <span class="badge bg-success" style="font-size:8px">QTY : {{$product_stocks['0']->stock}}</span>
                                      <?php } ?>
                                      <span class="badge bg-danger" style="font-size:8px" id='outofStock<?php echo $row->id;?>'></span>
                                </div>
                                
                                <div class='col-md-6'>
                                    
                                        @if(!empty($row->percent_discount))
                                            <div><span style='float:right;margin-right:2px;font-size:8px;margin-top:5px' class="badge bg-info">-{{$row->percent_discount}}&nbsp;% &nbsp;OFF</span></div>
                                         @else
                                            <div><span style='float:right;margin-right:2px;font-size:8px;margin-top:5px' class="badge bg-info">- {{Helper::get_currency()}}&nbsp;{{$row->flat_discount}}&nbsp;OFF</span></div>
                                         @endif
							...
							<div class="card-body">
								<h6 class="card-title cursor-pointer" style='font-size:12px'>{{$row->product_name}}</h6>
                            ...
                            <p class="mb-0 ms-auto">{{$row->rating}}</p>
								...
								<div class="clearfix">
									<p class="mb-0 float-start" style='font-size:10px'><strong style='font-size:10px'>{{$row->business_value}}</strong> BV</p>
									</div>
								<div class="d-flex align-items-center mt-3 fs-6">	
								<p class="mb-0 float-end fw-bold"><span class="me-2 text-decoration-line-through text-secondary">{{Helper::get_currency()}}&nbsp;{{$row->mrp}}</span><span>{{Helper::get_currency()}}&nbsp;{{$row->dp}}</span></p>
								</div>
								<div class='row'>
    								<div class="d-flex align-items-center mt-3 fs-6">
            ...
            <form id='myform' enctype='multipart/form-data'>
                <input type="hidden" class="form-control" name="from_id" id="from_id" value="{{$from_id}}">
                <div>
                      <label>* User Name:</label>
                      <div class="position-relative input-icon mt-2">
							<input type="text" class="form-control" onkeyup="search_user();" onchange="search_user();"   <?php if(!empty($user['0']->id)){?> value='<?php echo $user["0"]->first_name; ?>'<?php } ?> onmousedown="search_user();"  autocomplete='off'   name="customer" id="customer">
                  ...
                  <table class="table">
                     <thead>
                        <tr>
                          <th >Sr.No.</th>
                           ...
                           <th style="text-align:center">Total {{Helper::get_business_unit()}}</th>
                     ...
                     <tbody id="tablerow">
                        ...
                        <tr>
                            ...
                            <td style="text-align:center"><a onclick="decreaseqtys('{{$rows['id']}}')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px" type="text" min="1" value="{{$rows['quantity']}}" id="" readonly ><a onclick="increaseqtys('{{$rows['id']}}')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a></td>
                            ...
                            <td style="text-align:center"><a onclick="delete_Cart_Item({{$rows['id']}})"><i class="fa fa-trash"></i></a></td>
               ...
               <div style="margin-top:10px;" id='countinue_sale'>
                  ...
                  <input type="button"  onclick="uploaduserdata();" style="float:right;" class="btn btn-primary"  class="fixed-bottom" value="Continue Sale">
               </div>
               <?php } else { ?>
...
function add_to_cart(id){
            ...
            data: {'id':id,'cid':cid,'invoicedate':invoicedate,'from_id':from_id, "_token":"{{csrf_token()}}"},
            success: function (data) {
                 ...
                 $('.hidestockspan'+id).css('display', 'none');
                   ...
                   '<td style="text-align:center">'+i+'</td>'+
                    '<td style="text-align:center">'+value.name+'</td>'+
                     '<td style="text-align:center">'+'<a onclick="decreaseqtys('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px" type="text" min="1" value="'+value.quantity+'" id="quantity'+value.id+'" readonly ><a onclick="increaseqtys('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a>'+'</td>'+
                    '<td style="text-align:center">'+value.mrp+'</td>'+
                    '<td style="text-align:center">'+dp+'</td>'+
                    '<td style="text-align:center">'+total+'</td>'+
                    '<td style="text-align:center">'+total_bv+'</td>'+
                     '<td style="text-align:center">'+'<a onclick="delete_Cart_Item('+value.id+')"><i class="fa fa-trash"></i></a>'+'</td>'+
...
function increaseqtys(id){
            ...
            success: function (data) {
                    ...
                    '<td style="text-align:center">'+i+'</td>'+
                    '<td style="text-align:center">'+value.name+'</td>'+
                    '<td>'+'<a onclick="decreaseqtys('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px;background-color:#e7e7e7" type="text" min="1" value="'+value.quantity+'" id="quantity'+value.id+'" readonly ><a onclick="increaseqtys('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a>'+'</td>'+
                    '<td style="text-align:center">'+value.mrp+'</td>'+
                    '<td style="text-align:center">'+dp+'</td>'+
                    '<td style="text-align:center">'+total+'</td>'+
                    '<td style="text-align:center">'+total_bv+'</td>'+
        ...
        function decreaseqtys(id){
            ...
            success: function (data) {
                   ...
                   '<td style="text-align:center">'+i+'</td>'+
                    '<td style="text-align:center">'+value.name+'</td>'+
                    '<td style="text-align:center">'+'<a onclick="decreaseqtys('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>-</span></a><input name="quantity" style="width:50px;background-color:#e7e7e7" type="text" min="1" value="'+value.quantity+'" id="quantity'+value.id+'" readonly ><a onclick="increaseqtys('+value.id+')" class="btn btn-md" style="height: 26px;background-color:#e7e7e7"><span>+</span></a>'+'</td>'+
                    '<td style="text-align:center">'+value.mrp+'</td>'+
                    '<td style="text-align:center">'+dp+'</td>'+
                    '<td style="text-align:center">'+total+'</td>'+
                    '<td style="text-align:center">'+total_bv+'</td>'+
