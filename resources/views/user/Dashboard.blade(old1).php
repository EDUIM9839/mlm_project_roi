 ...
 @php
    ...
    <div class="page-content">
        ...
        <div class="row row-cols-1 row-cols-md-2 row-cols-xl-12 g-3 px-3">
            ...
            @endphp
          	<div class="col">
				<div class="card h-100 radius-10 border-0 bg-gradient-moonlit text">
					<div class="card-body h-100 pb-0">
					    
					      @php  $data = Auth::user();  @endphp
					    <div class="image-container">
					        <div class="image-box">
					            <img class="{{  $active ? 'active' : '' }}" src="{{ Storage::url('app/profileupload/').(Auth::user()->image ?? 'default-user.png')}}">    
					        </div>
					        
					            
					            
				         <div class="data">
				            <div class="name text-white">{{ $data->first_name }} {{$data->last_name}}</div>
				            <div class="id {{ $active ? 'active' : 'inactive' }}">{{ $data->userid }}</div>
				           ...
				           {{-- <div class="kyc_status"><b>KYC Status :</b> 
				             ...
				             <p class="text-white  small mb-1">
                                                        ...
                                                        @endphp
                                                       <b> Active on:</b> @if(!empty($user_package)){{Helper::formatted_date($user_package->created_at)}}@else <span style='color:#ff8a8a;'>dd-mm-YYYY</span> @endif</p>
                        
                       <div class="active-check-container" >
                           @if($active)
                ...
                <div class="col-12">
                    ...
                    <div class="mb-2 px-2">
					       <div class="mt-2 w-100 text-center text-white">
                                <strong>Current Progress:</strong> <span id="current-percent-text">0</span>%
                                <strong> / Target: 300%</strong>
                              </div>
                    ...
                    <div class="col-12 text-center">
                        <div class="d-flex align-items-center ms-4">
                            <span  class="link-area" style=; padding: 5px 10px;">
                               <span class='text-light'>Referal Link - <span  id="pwd_spn" style="color:#ff8d00;"> <?php echo $_SERVER['SERVER_NAME']; ?>/register?referal=<?php echo $data->userid; ?> </span></span> 
                            </span>
			 ...
			 <div class="col">
	                <div class="card h-100 current-position bg-gradient-moonlit text-white">
                    <div class="card-body pb-0">
                        <div class="d-flex justify-content-between align-items-start flex-wrap gap-2">
                            <div class="d-flex align-items-center text-white" style="font-weight: 600; width: 70%; border-bottom: 4px solid #73c666; padding-bottom: 12px; color: ##FFFFFF. !important;">
                                Wallet Summary
                            </div>
                            <div class="avatar" style="cursor: none;">
                                <i class="mdi-24px">
                                    <img src="/public/assets/images/icons/wallet.png" width="38px"></i>
                            </div>
                        </div>
                        <div class="card-info mt-2 pt-1">
                            <div class="row">
                              ...
                              <!-- Progress Bar -->
                              <!--<div class="progress">-->
                                <!--<div id="current-progress-bar" class="progress-bar bg-success" style="width: 0%">-->
                                <!--  0%-->
                                <!--</div>-->
                              <!--</div>-->
                         </div>
                              
                                <div class="col-6 mt-2 text-white" >Activation Wallet</div>
                                <div class="col-6  mt-2 text-end">
                                        <a href="javascript::void(0)" class="badge" style="background-color: #223c3f;">
                                            <span id="ctl00_cp_lblAllteam">{{Helper::get_currency()}}{{round(Auth::user()->saving_wallet,2)}}</span>
                                        </a>
                                   
                                </div>
                                
                                <div class="col-6 mt-2 text-white ">Incentive Wallet</div>
                                <div class="col-6  mt-2 text-end">
                                    <a href="javascript::void(0)" class="badge" style="background-color: #ff8d00c4;">
                                        {{Helper::get_currency()}}{{round(Auth::user()->incentive_wallet,2)}} 
                                    </a>
                               
                                </div>
                                <div class="col-6 mt-2 text-white ">ROI Wallet</div>
                                <div class="col-6  mt-2 text-end">
                                    <a href="javascript::void(0)" class="badge" style="background-color: #223c3f;">{{Helper::get_currency()}}{{round(Auth::user()->withdrawl_wallet,2)}} </a>
                                </div>
                                 <div class="col-6 mt-2 text-white ">Club Wallet</div>
                                <div class="col-6  mt-2 text-end">
                                    <a href="javascript::void(0)" class="badge" style="background-color: #ff8d00c4;">{{Helper::get_currency()}}{{round(Auth::user()->club_wallet,2)}}</a>
                                </div>
                                 <div class="col-6 mt-2 text-white ">Reward Wallet</div>
                                <div class="col-6  mt-2 text-end">
                                    <a href="javascript::void(0)" class="badge"  style="background-color: #223c3f; color: white;">{{Helper::get_currency()}}{{round(Auth::user()->reward_wallet,2)}}</a>
                                </div>
                                 <div class="col-6 mt-2 text-white">Current Reward</div>
                                      <div class="col-6 mt-2 text-end"> @php
                                     $rewardAchievedUser = DB::table("reward_achieved_user")->where("user_id", Auth::user()->id)->orderBy("level_no", "DESC")->first();
                                   @endphp
                               @if (!empty($rewardAchievedUser))
                                   @php
                                  $rewardVip = DB::table("reward_vip")->where("id", $rewardAchievedUser->level_no)->first();
                                  @endphp
                                    <a href="{{ route('reward') }}" class="badge"  style="background-color: #ff8d00c4;">{{ ucfirst($rewardVip->reward_name) }}</a>
                                   @else
                                   <span class="badge" style="background-color: #223c3f; color: white;">Pending</span>
                                   @endif
                                     </div>
                                 </div>
		    ...
		    <div class="col">
				<div class="card radius-10 bg-gradient-ohhappiness">
				 ...
				 <div class="card-body">
					<div class="d-flex align-items-center">
						<div>
    					   <h6 class="mb-1 text-white">Direct Income</h6>
    					   <h4 class="my-1 text-white">{{Helper::get_currency()}}{{number_format($direct_income, 2, '.', '')}}</h4>
    					</div>
						<div class="ms-auto">
					...
					<div class="d-flex align-items-center text-white">
						<p class="mb-0 font-13 text-white">Lapsed : {{Helper::get_currency()}} {{number_format($lapsdirect_income, 2, '.', '')}}</p>
        	 ...
        	 <div class="row row-cols-1 row-cols-md-2 row-cols-xl-4 px-3">
				...
				<div class="card radius-10 bg-gradient-deepblue">
				 ...
				 <div class="card-body">
					<div class="d-flex align-items-center">
						<div>
    					   <h6 class="mb-1 text-white">Club Income</h6>
    					     @php
                            $help_income=
                                DB::table('income_history')->where('received_user',Auth::user()->id)->where('type','club_income')->sum('amount');
                            @endphp
    					   <h4 class="my-1 text-white">{{Helper::get_currency()}} {{round($help_income,2)}}</h4>
    					</div>
						<div class="ms-auto">
                            <i class='bx bxs-wallet fs-3 text-white'></i>
						</div>
						...
						<div>
    					   <h6 class="mb-1 text-white">Self Investment</h6>
    					      @php
                                    ...
                                    @endphp
    					   <h4 class="my-1 text-white">{{Helper::get_currency()}} {{$total_invest_amount}}</h4>
    					</div>
						<div class="ms-auto">
                            <i class='bx bxs-wallet fs-3 text-white'></i>
						</div>
