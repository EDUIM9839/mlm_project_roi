                          ...
                          @endif
                            @foreach($result as $d)
                             ...
                             @php
            			       ...
            			       $image=DB::table('product_images')->where('pid',$d->product_id)->first();
            			     @endphp
            			  
			   
                                   <tr>
                                    <td>
                                      <img src="{{asset('productImages')}}{{'/'}}{{$image->image}}" style="width: 100px; height: 100px;">
                                      <span style="font-size:25px;">{{$da[0]->product_name}}</span>
                                    </td>
                                    <td>
                                         @if(!empty($da[0]->percent_discount))
                                        <p class="mt-4 fw-bold"><span class="me-2 text-decoration-line-through text-secondary ">{{Helper::get_currency()}}{{$da[0]->mrp}}</span> <span >{{Helper::get_currency()}}{{$da[0]->dp}}</span>
                                        </p>
                                        @elseif(!empty($da[0]->flat_discount))
                                         <p class="mt-4 fw-bold"><span class="me-2 text-decoration-line-through text-secondary ">{{Helper::get_currency()}}{{$da[0]->mrp}}</span> <span >{{Helper::get_currency()}}{{$da[0]->dp}}</span>
                                       </p>
                                        @else
                                         <p class="mt-4 fw-bold"><span >{{Helper::get_currency()}}{{$da[0]->dp}}</span>
                                       </p>
                                        @endif
                                       </td>
                                ...
                                @php $num=count($result);  $total=0; $bv=0; @endphp
                                    @foreach($result as $d)
                                 
                                    @php
            								    
                                    $ds=DB::table('products')->where('id',$d->product_id)->get();
                                    $total+=$ds[0]->dp*$d->quantity;
                                    $bv+=$ds[0]->business_value*$d->quantity;
                                    @endphp
                    ...
                    success: function(result)
                    {
                        console.log(result);
                            ...
                            if(result==1){ 
                        	    ...
                        	    msg: 'Product Deleted Successfully from Your Cart.'
                            	});
                            }else if(result==2){
                    ...
                    success: function(result)
                    {
                        console.log(result);
                            if(result==1){ 
                        		...
                        		position: 'top right',
                        		icon: 'bx bx-check-circle',
                        	    msg: 'Product Quantity Updated Successfully in Your Cart.'
                            	});
                            }else if(result==2){
                                Lobibox.notify('error', {
                        		pauseDelayOnHover: true,
                    ...
                    success: function(result)
                    {
                        console.log(result);
                            if(result==1){ 
                        		...
                        		position: 'top right',
                        		icon: 'bx bx-check-circle',
                        	    msg: 'Product Quantity Decreases Successfully in Your Cart.'
                            	});
                            }else if(result==2){
