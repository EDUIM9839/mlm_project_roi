				 ...
				 <div class="card">
					<div class="row g-0">
					  ...
					  <div class="col-md-8">
                       ...
                       @else
                        ...
                        @if($dataexists)
                        <div class='row'>
							  <left><a href="{{route('user_cart')}}" class="btn btn-outline-primary">Go to cart</a></left>
    				    </div>
					...
					<hr>
                     <div class="row row-cols-1 row-cols-lg-3">
                         @foreach($lastproduct as $hello)
						   <div class="col">
						    
							<div class="card">
								<div class="row g-0">
								  <div class="col-md-4">
								      <a href="{{route('product_details',['id'=>$hello->id])}}">
								           &nbsp;&nbsp;
										<center><img src="{{asset('productImages')}}{{'/'}}{{$hello->image}}" width="100" height="100" class="img-fluid" alt="loading..."></center>
										</a>
								  </div>
								  <div class="col-md-8">
									<div class="card-body">
									  <h6 class="card-title">{{$hello->product_name}}</h6>
									    <div class="d-flex align-items-center mt-3 fs-6">
                                                         <div class="cursor-pointer">
                                                             @for ($i=1; $i<=$hello->rating; $i++)
                                                                <i class="bx bxs-star text-warning"></i> 
                                                             @endfor
                                                             @for ($i=1;  $i<=(5-$hello->rating); $i++)
                                                                <i class="bx bxs-star text-secondary"></i> 
                                                             @endfor 
                                                              </div>
                                                 </div>
                                                  @if(!empty($hello->percent_discount))
									  <div class="clearfix">
										<p class="mb-0 float-start fw-bold"><span class="me-2 text-decoration-line-through text-secondary">{{Helper::get_currency()}}{{$hello->mrp}}</span><span>{{Helper::get_currency()}}{{$hello->dp}}</span></p>
									 </div>
									  @elseif(!empty($hello->flat_discount))
									  <div class="clearfix">
										<p class="mb-0 float-start fw-bold"><span class="me-2 text-decoration-line-through text-secondary">{{Helper::get_currency()}}{{$hello->mrp}}</span><span>{{Helper::get_currency()}}{{$hello->dp}}</span></p>
									 </div>
									   @else
									   <div class="clearfix">
										...
										<span>{{Helper::get_currency()}}{{$hello->dp}}</span></p>
									 </div>
									   @endif
							...
							<dd class="col-sm-9"> {{$hello->business_value}} {{Helper::get_business_unit()}}</dd>
						  </dl>
									</div>
								  </div>
								</div>
