...
class CrudsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->file('logo') && $request->file('fav_icon')) {
        ...
        DB::update('update business_setup set business_name = ?,email=?,country=?,phone=?,time_zone=?,time_format=?,currency_symbol=?,id_prefix=?, business_unit=?,address=?,footer_text=?,tax_amount=?,logo=?,fav_icon=?,opening_time=?,opening_day=?,closing_day=?,closing_time=?,pending_user_all_page_access=? where id = 1',
        ...
        $filename,$fav_icon,$opening_time,$opening_day,$closing_day,$closing_time,$parul]);
        ...
        [$business_name,$email,$country,$phone,$time_zone,$time_format,$currency_symbol,$id_prefix,$business_unit,$address,$footer_text,$tax_amount,$fav_icon,$opening_time,$opening_day,$closing_day,$closing_time,$parul]);
            ...
            //  dd("sanchit");
            DB::update('update business_setup set business_name = ?,email=?,country=?,phone=?,time_zone=?,time_format=?,currency_symbol=?,id_prefix=?,business_unit=?, address=?,footer_text=?,tax_amount=?,opening_time=?,opening_day=?,closing_day=?,closing_time=?,pending_user_all_page_access=? where id = 1',
        [$business_name,$email,$country,$phone,$time_zone,$time_format,$currency_symbol,$id_prefix,$business_unit,$address,$footer_text,$tax_amount,$opening_time,$opening_day,$closing_day,$closing_time,$parul]);
        }
    ...
    public function update_banner(Request $request){
        // $notification = $request->input('value');
         $file = $request->file('value');
        $filename = 'LOGO_'.time() . "." . $file->getClientOriginalExtension();
        // dd($filename);die;
        $path='profileupload/';
         $request->file('value')->storeAs($path, $filename);
        DB::update('update settings set value=? where id=1',[$filename]);
        // return redirect('notification');
  ...
  // code of All user edit 
     ...
     public function user_update(Request $request)
    {
       ...
       $id=$request->input('id');
       $paytm=$request->input('paytm_no');
       $gpay=$request->input('g_pay');
       $branch=$request->input('branch_detail');
       $phpne_pay=$request->input('phone_pay');
    ...
    //   dd($request->file('image'));
       if ($request->file('image')) {
        ...
        // $file->move(public_path("upload/userprofile/"), $filename);
         DB::table('user')->where('id',$id)->update(array('first_name' => $first_name, 'last_name' => $last_name,'contact'=>$contact, 'email'=>$email, 'city'=>$city, 'state'=> $state, 'country'=>$country,'address'=>$address, 'aadhar_no'=>$aadhar_no, 'bank_name'=>$bank_name, 'account_no'=>$account_no,'ifsc_code'=>$ifsc_code, 'account_holder_name'=>$account_holder_name, 'nomini_name'=>$nomini_name, 'nomini_contact'=>$nomini_contact, 'nomini_relation'=>$nomini_relation,'image'=>$filename,'pan'=>$pan, 'paytm_no'=>$paytm, 'g_pay'=>$gpay, 'branch_detail'=>$branch,'phone_pay'=>$phpne_pay,'tron_address'=>$tron_address));
       }else{
           DB::table('user')->where('id',$id)->update(array('first_name' => $first_name, 'last_name' => $last_name,'contact'=>$contact, 'email'=>$email, 'city'=>$city, 'state'=> $state, 'country'=>$country,'address'=>$address, 'aadhar_no'=>$aadhar_no, 'bank_name'=>$bank_name, 'account_no'=>$account_no,'ifsc_code'=>$ifsc_code, 'account_holder_name'=>$account_holder_name, 'nomini_name'=>$nomini_name, 'nomini_contact'=>$nomini_contact, 'nomini_relation'=>$nomini_relation,'pan'=>$pan, 'paytm_no'=>$paytm, 'g_pay'=>$gpay, 'branch_detail'=>$branch,'phone_pay'=>$phpne_pay,'tron_address'=>$tron_address)); 
       }
         Session::flash('message', "<p class='alert alert-success'>Record has been successfully Updated</p><br>");
    ...
    public function change_password_transaction (request $request)
    {
        ...
        $userid=$request->user_id;
        $pass=$request->password;
        $data = User::where('userid',$userid)->update(['transaction_password'=>$pass]);
        if($data)
        {
        $msg='Password has been successfully Updated';
         $request->session()->flash('success',$msg);
        ...
        return redirect()->route('change_password_transaction');
